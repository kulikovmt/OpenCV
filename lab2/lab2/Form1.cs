...
namespace lab2
{
    ...
    public partial class Form1 : Form
    {
        ...
        private ConvolutionKernelF convolutionKernel;
        ...
        private void button_open_image_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK) // открытие выбранного файла
            {
                ...
                comboBox_Channel.SelectedIndexChanged += (obj, args) =>
                {
                    ...
                    if (selectedIndex == 0) // Красный канал
                    {
                        // Отображаем красный канал в imageBox2
                        imageBox2.Image = sourceImage.Split()[2];
                    }
                    else if (selectedIndex == 1) // Зеленый канал
                    {
                        // Отображаем зеленый канал в imageBox2
                        imageBox2.Image = sourceImage.Split()[1];
                    }
                    else if (selectedIndex == 2) // Синий канал
                    {
                        // Отображаем синий канал в imageBox2
        ...
        private void button_grey_Click(object sender, EventArgs e)
        {
            ...
            for (int y = 0; y < sourceImage.Rows; y++)
            {
                ...
                {
                    // Применяем формулу для преобразования в серое изображение
        ...
        private Image<Bgr, byte> ApplySepiaFilter(Image<Bgr, byte> sourceImage)
        {
            ...
            // Применяем фильтр Sepia к каждому пикселю изображения
            for (int y = 0; y < sourceImage.Height; y++)
            {
                for (int x = 0; x < sourceImage.Width; x++)
                {
                    ...
                    // Вычисляем новые значения компонентов цвета для эффекта Sepia
                    ...
                    // Применяем интенсивность и глубину эффекта Sepia
                    blue = Math.Min(blue, 255);
                    green = Math.Min(green, 255);
                    red = Math.Min(red, 255);

                    // Устанавливаем новые значения компонентов цвета для пикселя
        ...
        private void button_medianBlur_Click(object sender, EventArgs e)
        {
            if (sourceImage != null)
            {
                int kernelSize = 15; // Размер ядра медианного фильтра
        ...
        //оконного фильтра (8)
        //private Image<Bgr, byte> ApplyFilter(Image<Bgr, byte> sourceImage)
        //{
        //    Image<Bgr, byte> resultImage = sourceImage.CopyBlank();

        //    // Проходим по каждому пикселю изображения, кроме краев
        //    for (int y = 1; y < sourceImage.Height - 1; y++)
        //    {
        //        for (int x = 1; x < sourceImage.Width - 1; x++)
        //        {
        //            double blue = 0, green = 0, red = 0;
        //            // Применяем фильтр к окрестности текущего пикселя
        //            for (int i = -1; i <= 1; i++)
        //            {
        //                for (int j = -1; j <= 1; j++)
        //                {
        ...
        //            // Обрезаем значения каналов, чтобы они находились в диапазоне от 0 до 255
        ...
        //            // Устанавливаем новые значения пикселя в результирующем изображении
